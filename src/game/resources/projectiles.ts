...
export function createProjectileManager(
	...
	deps: { onHit: (damage: number) => void },
): {
	spawn(from: PlanePoint, to: PlanePoint, damage: number): void
	...
	function spawn(from: PlanePoint, to: PlanePoint, damage: number): void {
		...
		projectiles.push({ id: nextId, from: start, to: end, damage, progress })
		nextId += 1
	}
